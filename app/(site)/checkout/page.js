...
export default function CheckoutPage() {
  ...
  const [ship, setShip] = useState(shippingMethods[0].id);
  const [placed, setPlaced] = useState(null);

  const shipping = shippingMethods.find((s) => s.id === ship) || shippingMethods[0];
  const total = subtotal + (shipping?.amount || 0) + ADMIN_FEE;
